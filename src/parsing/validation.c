...
int validate_values(t_game *game)
{
	...
	if (validate_path(game->textures.path_no, game) || validate_path(game->textures.path_so, game)
		|| validate_path(game->textures.path_we, game) || validate_path(game->textures.path_ea, game))
		return (1);
	if (validate_color(game->textures.str_f, game->textures.f, game) || validate_color(game->textures.str_c, game->textures.c, game))
		return (1);
	return (0);
}
